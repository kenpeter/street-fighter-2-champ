...
class Agent:
    ...
    def saveModel(self):
        ...
        model_path = os.path.join(Agent.DEFAULT_MODELS_DIR_PATH, self.getModelName() + ".weights.h5")
...
class DeepQAgent(Agent):
    ...
    def __init__(self, stateSize=32, load=False, resume=False, epsilon=1, name=None, moveList=Moves):
        """Initializes the agent and the underlying neural network
    ...
    def trainNetwork(self, data, model):
        ...
        max_samples = min(len(data), 100)  # Process at most 100 samples per training run
...
if __name__ == "__main__":
    ...
    testLobby.executeTrainingRun(episodes=args.episodes, background_training=args.background)